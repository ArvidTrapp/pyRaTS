...
sig_stat       = ts.timeseries(stat_gauss_0to200 + stat_gauss_200to400 + noise,t,unit = "m/s$^2$",var = "x", name = 'stationary time series')
sig_ns0to200   = ts.timeseries(nonstat_0to200    + stat_gauss_200to400 + noise,t,unit = "m/s$^2$",var = "x_{ns,lowf}", name = 'time series non-stationary in [0, 200] Hz')
sig_ns200to400 = ts.timeseries(stat_gauss_0to200 + nonstat_200to400    + noise,t,unit = "m/s$^2$",var = "x_{ns,highf}", name = 'time series non-stationary in [200, 400] Hz')

# combine sigs as list to make them iterable
sigs   = [sig_stat, sig_ns0to200, sig_ns200to400]
...
for sig in sigs:
    ...
    sig.plot_ls(method = [FLife.TovoBenasciutti])
    ''' estimation of non-stationarity matrix '''
    sig.est_nonstat(Nf = 100,olap=2,fu = 500,wfunc = sp.signal.windows.boxcar)
    sig.plot_nonstat(func=['Mxx','Axx'])
    print('excess kurtosis (timeseries):',sig.stat['kurtosis'])
    print('excess kurtosis (statistical):',sig.nonstat['kurtosis']) 
    
    # calculation of Fatigue Damage Spectrum (FDS)
    FDS_RFC.append(sig.est_fds(func = 'acc2acc',Nf =  60, m =  5)) 
    FDS_DK.append(sig.est_fds(func = 'acc2acc',Nf =  60, m =  5, method = FLife.Dirlik)) 
    
    # derive response signals for fD = 50
    f = sig.fpsd
    H = ts.get_tfSDOF(sig.fpsd,fD = 50, D = 0.05, func = 'acc2acc')
    ...
    ts.plot_nonstat(resp_nonstat,func=['Mxx'])
    print('excess kurtosis (timeseries): ', respsig.stat['kurtosis'])
    print('excess kurtosis (statistical): ', resp_nonstat['kurtosis'])   
    resps_gyy.append(resp_gyy)
    resps_nonstat.append(resp_nonstat) 
