...
step = 1/20
fpsd = np.arange(0,200+step,step)   # frequency vector for corresponding PSD

# stationary part 1
psd  = np.zeros(fpsd.shape)
psd[(fpsd >= 10) & (fpsd < 60)] = 1
inp.append({'fpsd': fpsd, 'psd': psd, 'T': 100, 'var': 'x_{p=1}'})

# stationary part 2
psd2 = np.zeros(fpsd.shape)
psd2[(fpsd >= 50) & (fpsd < 100)] = 1
inp.append({'fpsd': fpsd, 'psd': psd2, 'T': 200, 'var': 'x_{p=2}'})
...
respsig = sig.der_sdofResponse(fD = 50, D = 0.03,func = 'acc2dis')
respsig.plot()

# generate quasi-stationary realisation via list of multiple signal specfications
qssig = ts.timeseries(inp)
qssig2 = ts.timeseries([inp,inp])

qssig.plot()
qssig.plot(access = 'proc')
...
sig.est_psd(df = 0.5)    # estimate PSD via specification of frequency resolution
sig.est_stats()          # estimate some basic statistical properties
sig.est_ls()             # estimate load spectrum
...
# FDS (via FLife)
fdscalc1 = sig.est_fds(Nf = np.arange(5,50,5), D = 0.02,method = 'DK')          # Dirlik: 'DK' / Rainflow C.: RFC / FLife methods
fdscalc2 = sig.est_fds(Nf = np.arange(5,50,5), D = 0.02,method = fl.Narrowband) # Choice of FLife damage estimator methods
...
respsig2 = sig.der_sdofResponse(fD = 50, D = 0.03,func = 'acc2dis')
respsig2.plot()
...
asig = ts.get_analyticalSignal(sig.Xts,sig.ffper,30,40)
plt.figure()
plt.plot(t,abs(asig),'r')
# plt.show()  
asig = ts.get_posTaperedAnalyticalSignal(sig.Xts,sig.ffper,30,40)
plt.figure()
plt.plot(t,abs(asig),'r')
# plt.show()   
